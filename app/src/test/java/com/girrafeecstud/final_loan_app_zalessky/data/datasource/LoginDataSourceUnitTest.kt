...
class LoginDataSourceUnitTest {
    ...
    private lateinit var loginDataSourceImpl: LoginDataSourceImpl
    ...
    @Test
    fun `WHEN put correct login data EXPECT bearer token` () {

        runBlocking {

            enqueueSuccessResponse(fileName = "SuccessLoginResponse")

            val inputStream = javaClass?.classLoader.getResourceAsStream("SuccessLoginResponse")
            val source = inputStream.source().buffer()
            val expectedResult = source.readString(Charsets.UTF_8)
